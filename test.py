    ...
    if len(results.xyxy[0])>0:
      for i in range(len(results.xyxy[0])):
        if results.pandas().xyxy[0].iloc[i]['name']=='dog': # dog detected in the image
          if results.pandas().xyxy[0].iloc[i]['confidence'] > 0.5: # setting confidence threshold
          ...
          if results.pandas().xyxy[0].iloc[i]['confidence'] > 0.5: # setting confidence threshold
            ...
            rect = patches.Rectangle((xmin,ymin),xmax-xmin,ymax-ymin, linewidth=3, edgecolor='r', facecolor='none')
            ax.add_patch(rect)
            plt.text(xmin, ymin, 'cat'+' '+str(int(round(results.pandas().xyxy[0].iloc[i]['confidence']),3)*100)+'%', fontsize = 10,bbox = dict(facecolor = 'red', alpha = 0.5))
        elif results.pandas().xyxy[0].iloc[i]['name']=='human': # cat detected in the image
          if results.pandas().xyxy[0].iloc[i]['confidence'] > 0.5: # setting confidence threshold
            # get the bounding box
            xmin,ymin,xmax,ymax = results.pandas().xyxy[0].iloc[i]['xmin'],results.pandas().xyxy[0].iloc[i]['ymin'],results.pandas().xyxy[0].iloc[i]['xmax'],results.pandas().xyxy[0].iloc[i]['ymax'] 
            # crop the image
            ...
            rect = patches.Rectangle((xmin,ymin),xmax-xmin,ymax-ymin, linewidth=3, edgecolor='r', facecolor='none')
            ax.add_patch(rect)
            plt.text(xmin, ymin, 'human'+' '+str(int(round(results.pandas().xyxy[0].iloc[i]['confidence']),3)*100)+'%', fontsize = 10,bbox = dict(facecolor = 'red', alpha = 0.5)) 
