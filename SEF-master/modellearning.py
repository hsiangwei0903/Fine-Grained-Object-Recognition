...
def eval(model, dataloader=None, device='cpu', datasetname=None):
    ...
    if not datasetname or datasetname not in ['cubbirds', 'stcars', 'stdogs', 'vggaircraft', 'nabirds', 'wyzedogs', 'wyze', 'yt', 'google']:
    ...
    rsltparams['paths'] = paths
    
    
    
    # import json
    # json = json.dumps(list(rsltparams))
    # f = open("rsltparams-yt.json","w")
    # f.write(json)
    # f.close()
